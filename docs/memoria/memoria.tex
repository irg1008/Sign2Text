...
\def\maketitle{
  ...
  \begin{figure}[h!]
    \centering
    \includegraphics[width=0.4\textwidth]{../assets/logo/logo}
  \end{figure}
...
\newcommand{\titulo}{Sign2Text - Transcripción de lenguaje de signos (a nivel de palabra) mediante DL}
...
\begin{abstract}
  En la actualidad, no existe ningún traductor de lenguaje de signos a texto que funcione a tiempo real y sea capaz de traducir palabras complejas u oraciones largas. En este poyecto se busca usar un \loc{dataset} de personas realizando gestos y mapear los gestos a palabras individuales. Primero debemos preprocesar los datos de video para adptarlos a la entrada del modelo de \loc{deep learning}. Entonces entrenaremos el modelo y comprobaremos su rendimiento con ejemplos de test.

  El objetivo principal es habilitar la comunicación entre personas que usan lenguaje de signos y personas que no lo conocen. Adicionalmente, se pretende crear un modelo que pueda ser fácilmente adaptable a otros \loc{datasets} de video y que permita la futura traducción de signos en múltiples idiomas.

  Tras esto, queremos implementar una plataforma o demo web en la que los usuarios puedan probar el modelo con videos propios. Se habilitarán medios para que cualquier usuario y desarrollador pueda usar y mejorar el proyecto con el objetivo de que \project{Sign2Text} llegue al mayor número de manos posibles.
\end{abstract}

\renewcommand*\abstractname{Descriptores}
\begin{abstract}
  aprendizaje automático, aprendizaje supervisado, aprendizaje profundo, redes neuronales convolucionales, transcripción de languaje de signos, procesado de imagen y video, pytorch
\end{abstract}
...
\begin{abstract}
  The aim of this project is to use a dataset of people performing gestures and map the gestures to individual words. First we must preprocess the video data to adapt it to the input of the deep learning model. Then we will train the model and check its performance with test examples.
