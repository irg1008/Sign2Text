...
\begin{abstract}
  En la actualidad, no existe ningún traductor de lenguaje de signos a texto que funcione a tiempo real y sea capaz de traducir palabras complejas u oraciones extensas. En este proyecto se busca usar un \loc{dataset} de personas realizando gestos y mapear los gestos a palabras individuales. Primero debemos preprocesar los datos de vídeo para adaptarlos a la entrada del modelo de \loc{deep learning}. Entonces entrenaremos el modelo y comprobaremos su rendimiento con inferencia a tiempo real.
...
\begin{abstract}
  The aim of this project is to use a dataset of people performing gestures and map the gestures to individual words. First we must preprocess the video data to adapt it to the input of the deep learning model. Then we will train the model and check its performance with real-time inference.
